...
from leadr.common.domain.models import Entity, ImmutableEntity, _EntityBase
...
# Type variables for the shared base
_EntityBaseT = TypeVar("_EntityBaseT", bound=_EntityBase)
_ORMBaseT = TypeVar("_ORMBaseT")

# Type variables for mutable repository
...
class _RepositoryBase(ABC, Generic[_EntityBaseT, _ORMBaseT]):
    """Private shared base for BaseRepository and ImmutableBaseRepository.

    Contains common infrastructure: session management, ID extraction,
    abstract conversion methods, create, and all pagination helpers.
    """
    ...
    @abstractmethod
    def _to_domain(self, orm: _ORMBaseT) -> _EntityBaseT:
    ...
    @abstractmethod
    def _to_orm(self, entity: _EntityBaseT) -> _ORMBaseT:
    ...
    @abstractmethod
    def _get_orm_class(self) -> type[_ORMBaseT]:
    ...
    async def create(self, entity: _EntityBaseT) -> _EntityBaseT:
    ...
    def _extract_cursor_position(
        ...
        entity_id = str(orm.id)  # type: ignore[union-attr]
...
class BaseRepository(_RepositoryBase[DomainEntityT, ORMModelT]):
...
class ImmutableBaseRepository(_RepositoryBase[ImmutableEntityT, ImmutableORMT]):
