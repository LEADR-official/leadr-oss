...
@pytest.mark.asyncio
class TestRunEntryRepositoryPlayerNameAvailability:
    """Tests for RunEntryRepository.is_player_name_available method."""
    ...
    async def test_name_available_when_no_entries(self, db_session: AsyncSession):
        ...
        is_available = await entry_repo.is_player_name_available(
            board_id=fixtures["board_id"],
            player_name="Alice",
        )
    ...
    async def test_name_taken_by_other_identity(self, db_session: AsyncSession):
        ...
        # Create entry with player_name "Alice" for identity_1
        entry = RunEntry(
            board_id=fixtures["board_id"],
    ...
    async def test_case_insensitive_matching(self, db_session: AsyncSession):
        ...
        # Create entry with player_name "Alice"
        entry = RunEntry(
            board_id=fixtures["board_id"],
            identity_id=fixtures["identity_id_1"],
            score_event_id=fixtures["score_event_id_1"],
        ...
        is_available_lower = await entry_repo.is_player_name_available(
            board_id=fixtures["board_id"],
            player_name="alice",
            exclude_identity_id=fixtures["identity_id_2"],
        )
        ...
        is_available_upper = await entry_repo.is_player_name_available(
            board_id=fixtures["board_id"],
            player_name="ALICE",
            exclude_identity_id=fixtures["identity_id_2"],
        )
    ...
    async def test_different_board_allows_same_name(self, db_session: AsyncSession):
        ...
        # Create entry with player_name "Alice" on original board
        entry = RunEntry(
            board_id=fixtures["board_id"],
            identity_id=fixtures["identity_id_1"],
            score_event_id=fixtures["score_event_id_1"],
